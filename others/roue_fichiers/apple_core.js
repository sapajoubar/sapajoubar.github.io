...
if(g){g.parentNode.replaceChild(a,g)}}};var Element2={};Element2.Methods={getInnerDimensions:function(c){c=$(c);
...
}}};Object.extend(Element,Element2.Methods);if(typeof(AC)=="undefined"){AC={}}if(typeof(AC.Tracking)=="undefined"){AC.Tracking={}
...
g++){if(h.name===d){a[a.length]=h}}}return a}}AC.Storage={setItem:function(a,b,c){if(window.localStorage){localStorage.setItem(a,b)
...
},removeItem:function(a){if(window.localStorage){localStorage.removeItem(a)}this.eraseCookie(a)
},createCookie:function(c,d,e){if(e===null){e=365}if(e){var b=new Date();b.setTime(b.getTime()+(e*24*60*60*1000));
var a="; expires="+b.toGMTString()}else{var a=""}document.cookie=c+"="+d+a+"; path=/"
},readCookie:function(d){var f=d+"=";var a=document.cookie.split(";");for(var b=0;
b<a.length;b++){var e=a[b];while(e.charAt(0)==" "){e=e.substring(1,e.length)}if(e.indexOf(f)==0){return e.substring(f.length,e.length)
}}return null},eraseCookie:function(a){this.createCookie(a,"",-1)}};var gomez=gomez?gomez:{};
gomez.h3=function(c,a){for(var b in a){c[b]=a[b]}return c};gomez.h3(gomez,{b3:function(a){if(a<=0){return false
}return Math.random()<=a&&a},b0:function(d){var b=document.cookie;var a=b.match(new RegExp(";[ ]*"+d+"=([^;]*)"));
